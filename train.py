...
def move_all_files(src,dst):
    ...
    for file in os.listdir(src):
        ...
        if os.path.isfile(full_file_name):  # ????????(???????)?????
            shutil.copy(full_file_name, dst)  # shutil.copy??????????????  ?????????
    return
...
save_epoch_num = 100
# version = exp_name[-1:]
# version = re.search(r"_v(\d+)'", exp_name)
version = exp_name.split("_v")[1].split("'")[0]
# versions = re.search(r"_v(\d+)'", exp_name).group(1) if re.search(r"_v(\d+)'", exp_name) else None
...
#    if i%5==0 and i>45:
#        args["save_point"].append(i)

to_pil = transforms.ToPILImage()


print("pytroch???", torch.__version__)
# Path.
...
img_transform = transforms.Compose([
    transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1), #???brightness??????contrast??????saturation?????hue)
    transforms.ToTensor(),
...
def main(net):
    ...
    if args['optimizer'] == 'Adam':
        ...
        optimizer = optim.SGD([
            {'params': [param for name, param in net.named_parameters() if name[-4:] == 'bias'],
             'lr': 2 * args['lr'],'weight_decay': args['weight_decay']},
            {'params': [param for name, param in net.named_parameters() if name[-4:] != 'bias'],
             'lr': 1 * args['lr'], 'weight_decay': args['weight_decay']},
            # {'params': [param for name, param in net.named_parameters() if 'decoder' in name and name[-4:] == 'bias'],
            #  'lr': 2 * args['lr'], 'weight_decay': args['weight_decay']},
            # {'params': [param for name, param in net.named_parameters() if 'decoder' in name and name[-4:] != 'bias'],
            #  'lr': 1 * args['lr'], 'weight_decay': args['weight_decay']},
    ...
    if len(args['snapshot']) > 0:
        ...
        total_epoch = (args['epoch_num'] - int(args['last_epoch'])) * len(train_loader)
        print(total_epoch)
...
def train(net, optimizer):
    ...
    for epoch in range(args['last_epoch'] + 1, args['last_epoch'] + 1 + args['epoch_num']):
        ...
        for data in train_iterator:
            ...
            loss = 1 * loss_1 + 1 * loss_2 + 1 * loss_3 + 1 * loss_4 + 1 * loss_5
        ...
        if epoch in args['save_point']:
            ...
            net.cuda()
#            if epoch == 1 or epoch ==50 or epoch ==65:
#                val_model(exp_name, net, 384, results_path, pth_path, epoch, excel_path,args['save_predict_images'])
...
if __name__ == '__main__':
    ...
    net = MAMIFNet(pvt_name,dim=64)
    net = net.cuda().train()

#    from infer import val_model
